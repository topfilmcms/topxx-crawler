...
function get_first_episode_info() {
    $episode_list = get_post_meta(get_the_id(), 'ophim_episode_list', true);
    
    if (!is_array($episode_list) || empty($episode_list)) {
        return false;
    }
    
    $first_server_key = array_key_first($episode_list);
    if (empty($episode_list[$first_server_key]['server_data'])) {
        return false;
    }
    
    $first_episode = reset($episode_list[$first_server_key]['server_data']);
    if (empty($first_episode['name'])) {
        return false;
    }
    
    return [
        'server_key' => $first_server_key,
        'server_name' => $episode_list[$first_server_key]['server_name'],
        'episode_name' => $first_episode['name'],
        'episode_slug' => slugify($first_episode['name']),
        'link_embed' => $first_episode['link_embed'] ?? '',
        'link_m3u8' => $first_episode['link_m3u8'] ?? '',
    ];
}
function has_any_episodes() {
    return (bool) get_first_episode_info();
}
function get_first_episode_url() {
    $episode_info = get_first_episode_info();
    
    if (!$episode_info) {
        return '';
    }
    
    $getslug = get_option('ophim_watch_urls') ?: 'xem-phim';
    $post_slug = basename(get_permalink(get_the_id()));
    
    return home_url("/{$getslug}/{$post_slug}/tap-{$episode_info['episode_slug']}-sv-{$episode_info['server_key']}");
}
function watchUrl(){
...
/**
 * Trả về mảng URL ảnh preview (từ API TopXX, tối đa 6 ảnh) để hiển thị khi hover item.
 * Dùng cho theme: hover đổi ảnh preview.
 *
 * @return array Mảng URL ảnh, rỗng nếu không có.
 */
function op_get_preview_images()
{
    $v = get_post_meta(get_the_ID(), 'ophim_preview_images', true);
    if (!is_array($v)) {
        return array();
    }
    return array_values(array_filter(array_map('esc_url_raw', $v)));
}
