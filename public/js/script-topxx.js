...
jQuery(function ($) {
    ...
    // Bỏ qua thể loại Topxx: lưu vào file cache/filter_genre_topxx.json mỗi khi đổi checkbox (tránh cache JS)
    $("input[name='filter_genre_topxx[]']").on("change", function () {
        var arr = [];
        $("input[name='filter_genre_topxx[]']:checked").each(function () { arr.push($(this).val()); });
        $.ajax({
            url: ajaxurl,
            type: "POST",
            data: {
                action: "crawl_topxx_save_filter_genre",
                filterGenreTopxx: arr,
    ...
    var inputFilterGenreTopxx = [];

    buttonCrawlMovies.on("click", () => {
        divMsg.show(300);
        divMsgCrawlSuccess.show(300);
        divMsgCrawlError.show(300);
        inputFilterGenreTopxx = [];
        $("input[name='filter_genre_topxx[]']:checked").each(function () {
            inputFilterGenreTopxx.push($(this).val());
        });
        crawl_movies(false);
    });
    const crawl_movies = () => {
    ...
    $("#save_crawl_ophim_schedule").on("click", () => {
        ...
        let filterGenreTopxx = [];
        $("input[name='filter_genre_topxx[]']:checked").each(function () {
            filterGenreTopxx.push($(this).val());
        });
